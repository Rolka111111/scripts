...
tls="$(cat ~/log-install.txt | grep -w "SS-XRAY GRPC" | cut -d: -f2|sed 's/ //g')"
...
echo -e "Expired     : $exp"
echo -e "==============================="
echo -e "Config : ss://$(printf '%s' "${domain}-${user}@bughost.com:${tls}" | base64 -w 0)?ServiceName=xray-shadows-grpc&security=tls&encryption=chacha20-ietf-poly1305&host=${domain}&type=grpc&sni=${domain}#${domain}-${user}"
echo -e "==============================="
