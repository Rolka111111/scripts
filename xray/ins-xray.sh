...
sudo lsof -t -i tcp:80 -s tcp:listen | sudo xargs kill >/dev/null 2>&1
cd /root/
...
[Unit]
Description=Xray Service By JuraganSSH
...
[Unit]
Description=Trojan-Go Service By JuraganSSH
Documentation=https://t.me/mochammadhb
After=network.target nss-lookup.target
